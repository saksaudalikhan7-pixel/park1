...
export default function PrivacyPolicyPage() {
    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                const privacy = legalDocs.find(doc => doc.document_type === 'PRIVACY');
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const result = await updateLegalDocument(privacyPolicy.id, privacyPolicy);
            if (result.success) {
        ...
        return (
            <div className="space-y-8 max-w-4xl mx-auto pb-20">
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8">
                    <p className="text-slate-500">No privacy policy found in database.</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8 max-w-4xl mx-auto pb-20">
            <CMSBackLink />
            <div>
                <h1 className="text-2xl font-bold text-slate-900">Edit Privacy Policy</h1>
                <p className="text-slate-500">Modify privacy policy content</p>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 space-y-6">
                ...
                <div>
                    ...
                    <textarea
                        value={privacyPolicy.intro || ''}
                        onChange={(e) => setPrivacyPolicy({ ...privacyPolicy, intro: e.target.value })}
                        rows={3}
                        className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
                    />
                </div>
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">
                        Sections ({privacyPolicy.sections?.length || 0})
                    </label>
                            ...
                            <div key={index} className="border border-slate-200 rounded-lg p-4 space-y-3">
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setPrivacyPolicy({ ...privacyPolicy, sections: newSections });
                                    }}
